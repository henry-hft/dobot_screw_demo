...
class dobot_api_dashboard:
    ...
    def EnableRobot(self, param1, param2, param3, param4):
        string = f"EnableRobot({param1},{param2},{param3},{param4})"
        print(string)
    ...
    def GetErrorID(self):
        ...
        string = "GetErrorID()"
        self.socket_dashboard.send(str.encode(string,'utf-8'))
        data = self.WaitReply().decode('utf8')
        time.sleep (1)
        error_ids = re.findall(r'-?\d+', data)
        error_ids = [int(num) for num in error_ids if num != '0' or not any(char in data for char in ['[', ']'])]
        return error_ids
    ...
    def RobotMode(self):
        ...
        data = self.WaitReply()
        time.sleep (1)
        match = re.search(r'(?<=,{)(\d+)(?=},)', data.decode('utf8'))
        
        if match:
            modeNummber = int(match.group(1))
            return modeNummber
        else:
           return 0
    ...
    def GetPose(self):
        ...
        data =self.WaitReply()
        if(data):
            pose_str = data.decode('utf8').replace("'", '"').split(",")
            position = [pose_str[1][1:], pose_str[2], pose_str[3], pose_str[4]]
            return position
    ...
    def PauseScript(self):import socket
from threading import Timer
import numpy as np
import time
import re
